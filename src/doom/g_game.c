...
{ 
	...
	// Goblin Dice Rollaz: F11 - Validate demo
...
void G_ValidateDemo(void)
{
    ...
    {
        DEH_printf("Demo validation: No demo is currently playing or recording.\n");
        return;
    }
    ...
    if (demo_size < 16)
    {
        DEH_printf("ERROR: Demo too small (minimum 16 bytes).\n");
        goto cleanup;
    }

    version = *demo_ptr++;
    DEH_printf("Demo version byte: %d (0x%02X)\n", version, version);
    ...
    {
        DEH_printf("Format: Doom 1.9\n");
    }
    else if (version == 108)
    {
        DEH_printf("Format: Doom 1.9.1 (or compatible)\n");
    }
    else if (version == 109)
    {
        DEH_printf("Format: Doom 1.9.2+ (or compatible)\n");
    }
    ...
    DEH_printf("Players in demo: %d\n", valid_players);

    total_ticcmds = (demo_end - demo_ptr) / 4;
    DEH_printf("Estimated ticcmd count: %d\n", total_ticcmds);

    if (total_ticcmds < 1)
    {
        DEH_printf("WARNING: Demo has no ticcmds!\n");
    }
    else if (total_ticcmds < 100)
    {
        DEH_printf("Note: This is a very short demo.\n");
    }
    else if (total_ticcmds > 10000)
    {
        DEH_printf("Note: This is a long demo.\n");
    }

    if (demoplayback)
    {
        DEH_printf("Playback status: Active\n");
        DEH_printf("Current tic: %d\n", gametic);
    }
